...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    return errorMiddleware(async () => {
        ...
        const attachment = await clientsAttachmentsController.getById(params.id, userId)
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    return errorMiddleware(async () => {
        ...
        const deleted2 = await clientsAttachmentsController.destroy(params.id, userId)
        if (!deleted2) return responses.errors.notFound

        const updatedClient = await clientsController.getById(attachment.clientId, userId)
